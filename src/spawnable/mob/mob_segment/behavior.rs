    ...
    for entity in entities.iter() {
        ...
        loop {
            ...
            for pair in entity_pairs.iter_mut() {
                // add entities to mob segment entities if they are the mob, or their parent is in the vector already
        ...
        for (mob_segment_entity, mut mob_segment_component, _) in mob_segment_query.iter_mut() {
                ...
                .any(|check_entity| *check_entity == mob_segment_entity)
            {
                if let Some(disconnected_behaviors) = mob_segments_resource.mob_segments
                    [&mob_segment_component.mob_segment_type]
                    .disconnected_behaviors
                    .clone()
                {
                    mob_segment_component.behaviors = disconnected_behaviors.clone();
                }
            }
        }
    }
        ...
        match collision_event {
            ...
            SortedCollisionEvent::MobToMobSegmentContact {
                ...
                if entity == *mob_segment_entity && *mob_damage > 0 {
                    damage_dealt_event_writer.send(TakeDamageEvent {
                        damage: Damage::from_amount(*mob_damage),
                        target: *mob_segment_entity,
                    });
                }
            }
            SortedCollisionEvent::MobSegmentToMobSegmentContact {
                ...
                if entity == *mob_segment_entity_1 && *mob_segment_damage_2 > 0 {
                    damage_dealt_event_writer.send(TakeDamageEvent {
                        damage: Damage::from_amount(*mob_segment_damage_2),
