...
impl Plugin for UiPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(OnEnter(AppStates::GameOver), setup_game_over_system);

        app.add_systems(OnEnter(AppStates::Victory), setup_victory_system);

        app.add_systems(OnEnter(GameStates::Paused), setup_pause_system);
    }
}

// Consistently format mob+kill-count pairs.
fn print_mob_kills_from_data(data: &MobsKilledByPlayerCacheT) -> String {
    match (*data).get(&DEFAULT_USER_ID) {
        None => String::from("No mobs killed"),
        Some(mob_kill_counts) => mob_kill_counts
            .iter()
            .map(|(mob_type, n)| format!("{mob_type}: {n}"))
            .collect::<Vec<String>>()
            .join("\n"),
    }
}

/// A component that will cause the associated entity to bounce up and down at a rate determined by
