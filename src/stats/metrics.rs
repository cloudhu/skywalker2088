...
fn inc_usize_map<T: std::hash::Hash + Eq>(map: &mut HashMap<T, usize>, key: T) {
    match map.get(&key) {
        Some(v) => {
            map.insert(key, v + 1);
        }
...
/// Analogous to "log rolling" except we merge counters and add integers.
...
mod test {
    ...
    fn base_app_required_for_counting_metrics() -> App {
        let mut app = App::new();
        app.add_plugins((StatesPlugin, InputPlugin, PlayerPlugin, MinimalPlugins));
